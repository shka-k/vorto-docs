...
The modal stays open across installs, which run on a background thread. When an install finishes, the active buffer re-highlights automatically — no restart needed. You can also install or remove grammars inline:
...
When you open a file whose language has an install recipe but whose grammar isn't on disk, Vorto shows a yes/no **install this grammar?** prompt instead of a highlight-failed error. Press `y` or `Enter` to install — the buffer lights up once the install completes. Anything else dismisses the prompt and the file opens as plain text. Each grammar is asked about at most once per session, so opening more files of the same language stays quiet.
