...
In the modal, `j` / `k` move the selection, `Enter` installs, and `d` removes. Installs run in the background and re-highlight the active buffer on completion, so colors appear without a restart. See [Install and manage grammars](/languages/grammars#installing-grammars-from-the-editor).
