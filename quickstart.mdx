...
If you installed Vorto with [mise](/installation#install-with-mise) or a [prebuilt binary](/installation#download-a-prebuilt-binary), syntax highlighting is already on — those binaries ship with grammars for every built-in language embedded, extracted on first launch. You can skip ahead to [the next step](#next-steps).
